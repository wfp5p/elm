...
/* $Id: elm_defs.h,v 1.7 1999/03/24 14:03:42 wfp5p Exp $ */
...
struct header_rec {
	...
	char to[SLEN];	/** who it was sent to		**/
...
EXTERN int debug;			/* debugging verbosity (0=off)	*/
EXTERN FILE *debugfile;   	        /* file for debut output	*/
EXTERN char *user_name;			/* user name, from passwd	*/
EXTERN char *user_home;			/* home directory, from passwd	*/
EXTERN char user_fullname[SLEN];	/* full username, from passwd	*/
EXTERN char host_name[SLEN];		/* uucp name of local machine	*/
EXTERN char host_domain[SLEN];		/* local domain with leading dot*/
EXTERN char host_fullname[SLEN];	/* local FQDN to use for mail	*/
EXTERN char *incoming_folder;		/* default folder ($MAIL)	*/
EXTERN nl_catd elm_msg_cat;		/* message catalog		*/
EXTERN struct addr_rec
	    *alternative_addresses;	/* other addrs where we get mail*/
